...
public class DialogManager {
     ...
     * @return
     */
    private static Dialog createLoadingDialog(String msg) {
        LayoutInflater inflater = LayoutInflater.from(ActivityManager.getInstances().getTopActivity());
        ...
        // 创建自定义样式dialog
        sLoadingDialog = new Dialog(ActivityManager.getInstances().getTopActivity(), R.style.style_loading_dialog);
        sLoadingDialog.setCancelable(true);// 不可以用“返回键”取消
        sLoadingDialog.setContentView(v, new LinearLayout.LayoutParams(UIUtils.dip2px(ActivityManager.getInstances().getTopActivity(), 100), UIUtils.dip2px(ActivityManager.getInstances().getTopActivity(), 100)));// 设置布局
